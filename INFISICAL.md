...
| `BRIEFS_AUTH_SECRET` | System API and MCP bearer validation |
| `BRIEFS_SESSION_SECRET` | Daily session cookies |
| `BRIEFS_RESEND_API_KEY` | Production OTP delivery |

`BRIEFS_EMAIL_FROM` is non-secret runtime configuration and belongs in the deploy environment
file. `BRIEFS_DATABASE_URL` is generated by `deploy.sh` from the Postgres password unless an
