...
2. **Per-user isolation** — Every graph node, edge, and action is scoped to a single `userId`. Cross-tenant access is rejected.
...
Brief will **not** use Supabase. External tool auth stays with the user's MCPs; Brief only validates MCP access tokens.
